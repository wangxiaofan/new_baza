 ...
 */
@TableAnnotation(tableName = "companySearchFilterTable")
public class CompanySearchFilterInfoBean implements Serializable {
    ...
    @ColumnAnnotation(columnName = "yearOfExperienceRegion", columnType = "INTEGER")
    public int yearOfExperienceRegion;

    @ColumnAnnotation(columnName = "searchString")
...
//    }
    ...
    public void attachWorkYearFilter(WorkYearFilterBean mWorkYearFilter) {
        mayChanged = true;
        this.yearOfExperienceRegion = mWorkYearFilter.yearParameter;
        this.workYearName = mWorkYearFilter.name;
    }

    public void attachCompanyFilter(String company) {
        mayChanged = true;
        this.company = company;
    }

    public void attachTitleFilter(String title) {
        mayChanged = true;
        this.title = title;
    }

    public void attachSchool(String schoolName, int schoolParameter) {
        mayChanged = true;
        this.schoolName = schoolName;
        this.schoolParameter = schoolParameter;
    }
...
//    }
    ...
    public boolean shouldProcessChanged() {
        if (id == null || searchString == null || mayChanged) {
            ...
            if (yearOfExperienceRegion > 0) {
                stringBuilder.append(workYearName).append(doll);
                isJustDefault = false;
            }
            if (degreeParameter > 0) {
                stringBuilder.append(degreeName).append(doll);
                isJustDefault = false;
            }
            if (!TextUtils.isEmpty(company)) {
                isJustDefault = false;
                stringBuilder.append(company).append(doll);
            }
            if (!TextUtils.isEmpty(title)) {
                stringBuilder.append(title).append(doll);
                isJustDefault = false;
            }
            ...
            if (stringBuilder.length() > 0) {
                ...
                id = MD5.getStringMD5(searchString);
            }
    ...
    public static void setParameterToMap(CompanySearchFilterInfoBean mSearchFilterInfo, HashMap<String, String> mSearchParam, boolean cloudSearch) {
...
//            LogUtil.d("tagBindingList  = " + tagBindingList);
    ...
    public static void setParameterToMap(CompanySearchFilterInfoBean mSearchFilterInfo, HashMap<String, String> mSearchParam) {
        setParameterToMap(mSearchFilterInfo, mSearchParam, false);
    }

    private boolean isMoreFilterEnable() {
        return ((startYearParameter != FILTER_INT_NONE || endYearParameter != FILTER_INT_NONE || degreeParameter != FILTER_INT_NONE || schoolParameter != FILTER_INT_NONE || sexParameter != FILTER_INT_NONE) || !TextUtils.isEmpty(sourceParameter) || !TextUtils.isEmpty(company) || !TextUtils.isEmpty(title));
    }
    ...
    private boolean isAdvanceSearchEnable() {
        return (cityCode != LocalAreaBean.CODE_NONE) || isMoreFilterEnable() || (labelList != null && !labelList.isEmpty() || yearOfExperienceRegion > 0 || degreeParameter > 0);
    }
