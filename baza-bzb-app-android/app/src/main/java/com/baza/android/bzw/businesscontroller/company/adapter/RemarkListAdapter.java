...
import com.baza.android.bzw.businesscontroller.resume.detail.adapter.remark.AudioRemarkStateProvider;
...
public class RemarkListAdapter extends BaseBZWAdapter implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    private int getItemLayoutId(int itemType) {
        ...
        switch (itemType) {
            case TYPE_TEXT:
                layoutId = R.layout.adapter_item_for_text_remark_company;
                break;
            case TYPE_VOICE:
                layoutId = R.layout.adapter_item_for_voice_remark_company;
                break;
        }
        return layoutId;
    }
    ...
    private void setTextRemarkView(ViewHolder viewHolder, RemarkBean data, int position) {
        if (data.userName.contains("@")) {
            String[] str = data.userName.split("@");
            String textSource = String.format("%1$s <font color='#94A1A5'><small>%2$s</small></font>", str[0], "@" + str[1]);
            viewHolder.mTextHolder.tv_user_name.setText(Html.fromHtml(textSource));
        } else {
            viewHolder.mTextHolder.tv_user_name.setText(data.userName);
        }
        if (!StringUtil.isEmpty(data.avatar)) {
            LoadImageUtil.loadBase64Image(data.avatar, R.drawable.avatar_def, viewHolder.mTextHolder.userHead);
        } else if (!StringUtil.isEmpty(data.defaultAvatar)) {
            LoadImageUtil.loadImage(data.defaultAvatar, R.drawable.avatar_def, viewHolder.mTextHolder.userHead);
        } else {
            viewHolder.mTextHolder.userHead.setBackgroundResource(R.drawable.avatar_def);
        }
    ...
    private void setVoiceRemarkView(ViewHolder viewHolder, RemarkBean data, int position) {
        ...
        if (data.userName.contains("@")) {
            String[] str = data.userName.split("@");
            String textSource = String.format("%1$s <font color='#94A1A5'><small>%2$s</small></font>", str[0], "@" + str[1]);
            viewHolder.mVoiceHolder.tv_user_name.setText(Html.fromHtml(textSource));
        } else {
            viewHolder.mVoiceHolder.tv_user_name.setText(data.userName);
        }
        if (!StringUtil.isEmpty(data.avatar)) {
            LoadImageUtil.loadBase64Image(data.avatar, R.drawable.avatar_def, viewHolder.mVoiceHolder.userHead);
        } else if (!StringUtil.isEmpty(data.defaultAvatar)) {
            LoadImageUtil.loadImage(data.defaultAvatar, R.drawable.avatar_def, viewHolder.mVoiceHolder.userHead);
        } else {
            viewHolder.mVoiceHolder.userHead.setBackgroundResource(R.drawable.avatar_def);
        }
        ...
        viewHolder.mVoiceHolder.imageView_edit.setVisibility(data.isSelfCreated() ? View.VISIBLE : View.GONE);
    ...
    private static class ViewHolder {
        ...
        static class TextHolder {
            TextView textView_content, textView_remark_time, textView_jobHoping, textView_hirerDes, textView_expectSalary;
            ...
            private TextHolder(View convertView) {
                ...
                userHead = convertView.findViewById(R.id.user_head);
        ...
        ViewHolder.TextHolder mTextHolder;
        ViewHolder.VoiceHolder mVoiceHolder;

        ViewHolder(View convertView, int itemType, Handler mHandler) {

            switch (itemType) {
                case TYPE_TEXT:
                    mTextHolder = new ViewHolder.TextHolder(convertView);
                    break;
                case TYPE_VOICE:
                    mVoiceHolder = new ViewHolder.VoiceHolder(convertView);
                    break;
            }
        }
    }
