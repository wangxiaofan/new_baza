...
import com.tencent.qcloud.tim.uikit.modules.chat.FileDetailsActivity;
...
public class MessageFileHolder extends MessageContentHolder {
    ...
    @Override
    public void layoutVariableViews(final MessageInfo msg, final int position, ElementBean elementBean) {
        TIMElem elem = msg.getElement();
        if (!(elem instanceof TIMFileElem)) {
            return;
        }
        final TIMFileElem fileElem = (TIMFileElem) elem;
        //msg.setDataPath(TUIKit.getAppContext().getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath());
        final String path = msg.getDataPath();
        Log.e("herb", "最初文件路径>>" + path);
        fileNameText.setText(fileElem.getFileName());
        final String size = FileUtil.FormetFileSize(fileElem.getFileSize());
        fileSizeText.setText(size);
        msgContentFrame.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //跳转到文件详情页
                Intent intent = new Intent(TUIKit.getAppContext(), FileDetailsActivity.class);
                intent.putExtra("name", fileElem.getFileName());
