...
public class MessageLayout extends MessageLayoutUI {
    ...
    private void initPopActions(final MessageInfo msg) {
        ...
        if (msg.isSelf()) {
            ...
            //判断消息时间和当前时间是否超过两分钟
            if (System.currentTimeMillis() - msg.getMsgTime() * 1000 <= 2 * 60 * 1000) {
                action = new PopMenuAction();
    ...
    public void postSetAdapter(MessageListAdapter adapter) {
        mAdapter.setOnClickListener(new MessageLayout.OnClickListener() {
            @Override
            public void onMessageClick(View view, int position, MessageInfo messageInfo) {
                if (mOnClickListener != null) {
                    mOnClickListener.onMessageClick(view, position, messageInfo);
                }
            }
        });
