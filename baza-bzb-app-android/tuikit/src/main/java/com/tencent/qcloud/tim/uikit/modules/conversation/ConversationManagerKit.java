...
public class ConversationManagerKit implements TIMRefreshListener, MessageRevokedManager.MessageRevokeHandler {
    ...
    public void loadConversation(IUIKitCallBack callBack) {
        ...
        Log.e("herb", "获取原始聊天列表长度>>" + TIMConversations.size());
        ...
        for (int i = 0; i < TIMConversations.size(); i++) {
            ...
            Log.e("herb", i + ">>conversationInfo>>" + conversationInfo);
            if (conversationInfo != null) {
                conversationInfo.setType(ConversationInfo.TYPE_COMMON);

                //未读总数
                Configs.mUnreadTotal = Configs.mUnreadTotal + conversationInfo.getUnRead();
        ...
        Log.e("herb", "处理之后的数据>>" + infos.size());
        //更新消息未读总数
        updateUnreadTotal(Configs.mUnreadTotal);
        if (callBack != null) {
    ...
    public void onRefreshConversation(List<TIMConversation> conversations) {
        ...
        Log.e("herb", "onRefreshConversation conversations:" + conversations);
        if (mProvider == null) {
            return;
        }
        ArrayList<ConversationInfo> infos = new ArrayList<>();
...
//                            InputLayout.id = group;
//
//                            ChatInfo chatInfo = new ChatInfo();
//                            chatInfo.setType(TIMConversationType.Group);//个人对话还是群组对话
//                            chatInfo.setId(group);//对话ID
//                            chatInfo.setChatName("das");//对话标题
//
//                            String url = "scheme://baza/IMChatActivity";//这个就是刚刚前面在AndroidManManifest中设置的，?后面是需要传去的参数，但是不要太长
...
//                            TUIKit.getAppContext().startActivity(intent);
        ...
        for (int j = 0; j < infos.size(); j++) {
            ...
            for (int i = 0; i < dataSource.size(); i++) {
                ...
                if (cacheInfo.getId().equals(update.getId()) && cacheInfo.isGroup() == update.isGroup()) {
                    ...
                    //infos.remove(j);
                    //需同步更新未读计数
                    Configs.mUnreadTotal = Configs.mUnreadTotal - cacheInfo.getUnRead() + update.getUnRead();
                    TUIKitLog.v(TAG, "onRefreshConversation after mUnreadTotal = " + Configs.mUnreadTotal);
                    exist = true;
                    break;
                }
            }
            if (!exist) {
                Configs.mUnreadTotal += update.getUnRead();
                TUIKitLog.i(TAG, "onRefreshConversation exist = " + exist + ", mUnreadTotal = " + Configs.mUnreadTotal);
            }
        }
        updateUnreadTotal(Configs.mUnreadTotal);
    ...
    private ConversationInfo TIMConversation2ConversationInfo(final TIMConversation conversation) {
        ...
        list = MessageInfoUtil.TIMMessage2MessageInfo(message, isGroup);

        if (list != null && list.size() > 0) {
            //Log.e("herb", "最后一条消息>>" + list.get(list.size() - 1).toString());
        ...
        //取出没有聊天ID的
        if (conversation.getPeer() == null || conversation.getPeer().equals("")) {
            return null;
        }
    ...
    public void deleteConversation(int index, ConversationInfo conversation) {
        ...
        if (status) {
            ...
            updateUnreadTotal(Configs.mUnreadTotal - conversation.getUnRead());
        }
    }
    ...
    public void deleteConversation(String id, boolean isGroup) {
        ...
        for (int i = 0; i < conversationInfos.size(); i++) {
            ConversationInfo info = conversationInfos.get(i);
            if (info.getId().equals(id)) {
                updateUnreadTotal(Configs.mUnreadTotal - info.getUnRead());
                break;
            }
        }
        if (mProvider != null) {
    ...
    public void updateUnreadTotal(int unreadTotal) {
        ...
        Configs.mUnreadTotal = unreadTotal;
        for (int i = 0; i < mUnreadWatchers.size(); i++) {
            mUnreadWatchers.get(i).updateUnread(Configs.mUnreadTotal);
        }
    }
