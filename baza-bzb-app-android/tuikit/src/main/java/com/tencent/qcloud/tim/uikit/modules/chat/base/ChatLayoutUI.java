...
public abstract class ChatLayoutUI extends LinearLayout implements IChatLayout {
    ...
    public void setChatInfo(ChatInfo chatInfo) {
        ...
        if (chatInfo.getType() == TIMConversationType.C2C) {
            if (chatInfo.getCompany() != null && !chatInfo.getCompany().equals("")) {
                getTitleBar().getmCenterTitleMin().setVisibility(VISIBLE);
                getTitleBar().setTitle("(" + chatInfo.getCompany() + "|" + chatInfo.getPost() + ")", TitleBarLayout.POSITION.SECOND);
            }
        }
    }
