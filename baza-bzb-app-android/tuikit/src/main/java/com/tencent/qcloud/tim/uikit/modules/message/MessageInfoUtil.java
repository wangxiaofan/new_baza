...
public class MessageInfoUtil {
     ...
     * @return
     */
    public static List<MessageInfo> TIMMessages2MessageInfos(final List<TIMMessage> timMessages, final boolean isGroup) {
        Log.e("herb", ">>timMessages.size>>" + timMessages);

        if (timMessages == null) {
            return null;
        }
        final List<MessageInfo> messageInfos = new ArrayList<>();

        for (int i = 0; i < timMessages.size(); i++) {
            List<MessageInfo> info;
            info = TIMMessage2MessageInfo(timMessages.get(i), isGroup);
            if (info != null) {
    ...
    private static MessageInfo ele2MessageInfo(final MessageInfo msgInfo, TIMMessage timMessage, TIMElem ele, boolean isGroup) {
        ...
        if (type == TIMElemType.Custom) {
            Log.e("herb", "消息类型>>TIMElemType.Custom");
            TIMCustomElem customElem = (TIMCustomElem) ele;

            if (customElem != null && customElem.getData() != null) {
                try {
                    ElementBean elementBean = new Gson().fromJson(new String(customElem.getExt(), "UTF-8"), ElementBean.class);
                    if (elementBean != null) {
                        if (elementBean.getType().equals("atMessage")) {
                            //@消息
                        }
                        Log.e("herb", "消息类型>>" + elementBean.toString());
                    }
                } catch (Exception e) {
                }
            }
            ...
            if (data.equals(GROUP_CREATE)) {
                ...
                //Log.e("herb", "群聊信息获取01>>" + msgInfo.getGroupNameCard());
                //Log.e("herb", "群聊信息获取02>>" + msgInfo.getFromUser());
                Log.e("herb", "msgInfo.getGroupNameCard()>>" + msgInfo.getGroupNameCard());
                Log.e("herb", "msgInfo.getFromUser()>>" + msgInfo.getFromUser());

                /*String message = TUIKitConstants.covert2HTMLString(
                        TextUtils.isEmpty(msgInfo.getGroupNameCard())
                                ? name
                                : msgInfo.getGroupNameCard()) + "发起了群聊";*/
                String message = "发起群聊";
                if (msgInfo.getGroupNameCard() == null || msgInfo.getGroupNameCard().equals("")) {
                    message = TUIKitConstants.covert2HTMLString(Configs.nameIdsMap.get(msgInfo.getFromUser())) + message;
                } else {
                    message = TUIKitConstants.covert2HTMLString(msgInfo.getGroupNameCard()) + message;
                }
                Log.e("herb", "message>>" + message);
            ...
            Log.e("herb", "消息类型>>TIMElemType.GroupTips");
            final TIMGroupTipsElem groupTips = (TIMGroupTipsElem) ele;
            final TIMGroupTipsType tipsType = groupTips.getTipsType();
            if (groupTips.getChangedGroupMemberInfo().size() > 0) {
                Log.e("herb", "groupTips的size>0");
                ...
                nameSetExt(user, tipsType, msgInfo, groupTips, groupTips.getOpUser());
            } else {
                Log.e("herb", "groupTips.getOpUserInfo()>>" + groupTips.getOpUserInfo().toString());
                //更新名字
                String user = groupTips.getOpUserInfo().getIdentifier();
                nameSetExt(user, tipsType, msgInfo, groupTips, groupTips.getOpUser());
            }
        } else {
            Log.e("herb", "消息类型>>其他");
            if (type == TIMElemType.Text) {
                ...
                Log.e("herb", "TIMFileElem>>fileElem.getPath>>" + fileElem.getPath());
                Log.e("herb", "TIMFileElem>>fileElem.getFileName>>" + fileElem.getFileName());

//                 filename = fileElem.getUuid();
//                if (TextUtils.isEmpty(filename)) {
//                    filename = System.currentTimeMillis() + fileElem.getFileName();
//                    filename = fileElem.getFileName();
//                }

                //final String path = TUIKitConstants.FILE_DOWNLOAD_DIR + filename;
                final String path = TUIKit.getAppContext().getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath() + "/" + filename;
                Log.e("herb", "MessageInfoUtil>>原始路径>>path>>" + path);
                File file = new File(path);
                if (file.exists()) {
                    if (msgInfo.isSelf()) {
        ...
        if (timMessage.status() == TIMMessageStatus.HasRevoked) {
            ...
            if (msgInfo.isSelf()) {
                ...
                //String message = name;
                msgInfo.setExtra(Configs.nameIdsMap.get(msgInfo.getFromUser()) + "撤回了一条消息");
            } else {
    ...
    //更换名字后
    private static void nameSetExt(String user, TIMGroupTipsType tipsType, MessageInfo msgInfo, TIMGroupTipsElem groupTips, String opUser) {
        Log.e("herb", "nameSetExt>>user>>>" + user);
        Log.e("herb", "nameSetExt>>opUser>>>" + opUser);
        Log.e("herb", "Configs.nameIdMaps长度>>" + Configs.nameIdsMap.size());

        user = Configs.nameIdsMap.get(user);
        opUser = Configs.nameIdsMap.get(opUser);
        //String message = user;
        String message = TUIKitConstants.covert2HTMLString(user);
        opUser = TUIKitConstants.covert2HTMLString(opUser);
        if (tipsType == TIMGroupTipsType.Join) {
            msgInfo.setMsgType(MessageInfo.MSG_TYPE_GROUP_JOIN);
            message = opUser + "将" + message + "加入群组";
        }
