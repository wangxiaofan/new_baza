...
public class ReflectorUtil {
    ...
    private static boolean isV4ViewPagerLoaded = true;
    private static boolean isV4SwipeRefreshLayoutLoaded = true;
//    private static boolean isV7AlertDialogLoaded = true;
    private static boolean isV4FragActivityLoaded = true;
    private static boolean isV4FragmentLoaded = true;
    private static boolean isV4FragmentControllerLoaded = true;
        ...
        if ((!isV7RecyclerViewLoaded) || ((!clazz.equals(RecyclerView.class)) && (!isV7RecyclerViewCached) && (paramString.contains("RecyclerView")))) {
            try {
                ...
                if ((classRecyclerView != null) && (methodItemPosition != null)) {
                    methodLayoutManager = classRecyclerView.getDeclaredMethod("getLayoutManager");
                    Class classLayoutManager = classRecyclerView.getClassLoader().loadClass(classRecyclerView.getName() + "$LayoutManager");
                    methodCanScrollHorizontally = classLayoutManager.getMethod("canScrollHorizontally");
                    methodCanScrollVertically = classLayoutManager.getMethod("canScrollVertically");
    ...
    private static Class getClassRecyclerView(Class clazz) {
        while ((clazz != null) && (!clazz.equals(ViewGroup.class))) {
            try {
                methodItemPosition = clazz.getDeclaredMethod("getChildAdapterPosition", View.class);
            } catch (NoSuchMethodException e) {
            }
            if (methodItemPosition == null) {
                try {
                    methodItemPosition = clazz.getDeclaredMethod("getChildPosition", View.class);
                } catch (NoSuchMethodException e) {
                }
            }
            if (methodItemPosition != null) {
    ...
    static {
        try {
            ...
            localClass.getDeclaredMethod("getChildAdapterPosition", View.class);
        } catch (ClassNotFoundException e) {
        ...
        try {
            sClassSwipeRefreshLayout = Class.forName("androidx.viewpager.widget.ViewPager.SwipeRefreshLayout");
        } catch (ClassNotFoundException e) {
...
//            isV7AlertDialogLoaded = false;
        ...
        try {
            Class clazz = Class.forName("androidx.viewpager.widget.ViewPager.app.FragmentActivity");
            fieldmFragments = clazz.getDeclaredField("mFragments");
        } catch (ClassNotFoundException e) {
        ...
        try {
            Class.forName("androidx.viewpager.widget.ViewPager.app.Fragment");
        } catch (ClassNotFoundException e) {
            isV4FragmentLoaded = false;
        }
        try {
            Class.forName("androidx.viewpager.widget.ViewPager.app.FragmentController");
        } catch (ClassNotFoundException e) {
