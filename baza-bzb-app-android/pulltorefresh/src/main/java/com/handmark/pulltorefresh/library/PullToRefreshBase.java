...
import com.handmark.pulltorefresh.library.internal.BazaHeaderLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        if (onDispatchTouchListener != null)
            onDispatchTouchListener.onTouch(this, event);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (mState == State.RELEASE_TO_REFRESH && ((mCurrentMode == Mode.PULL_FROM_START && headReboundInsteadRefresh) || (mCurrentMode == Mode.PULL_FROM_END && foodReboundInsteadLoad))) {
    ...
    protected void headReboundRelease() {
    }

    protected void footReboundRelease() {
    }

    public void setHeadReboundInsteadRefresh(boolean headReboundInsteadRefresh) {
        this.headReboundInsteadRefresh = headReboundInsteadRefresh;
        if (mHeaderLayout != null)
            mHeaderLayout.reboundState(headReboundInsteadRefresh);
    }

    public void setFootReboundInsteadLoad(boolean foodReboundInsteadLoad) {
        this.foodReboundInsteadLoad = foodReboundInsteadLoad;
        if (mFooterLayout != null)
            mFooterLayout.reboundState(foodReboundInsteadLoad);
    }

    private boolean headReboundInsteadRefresh, foodReboundInsteadLoad;
    ...
    protected final void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            mScrollingWhileRefreshingEnabled = bundle.getBoolean(STATE_SCROLLING_REFRESHING_ENABLED, true);
    ...
    public enum AnimationStyle {
        ...
        LoadingLayoutBase createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                ...
                case SELF_DEFINE:
                    return new BazaHeaderLayout(context, 0);
            }
        }
    }
