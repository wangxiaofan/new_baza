...
public class PullToRefreshListView extends PullToRefreshAdapterViewBase<ListView> {
    ...
    private FrameLayout frameLayout_bottomResultHint;
    private boolean mListViewExtrasEnabled = true;

    private boolean mAllLoadTagEnabled = true;

    public void setAllLoadTagEnabled(boolean allLoadTagEnabled) {
        this.mAllLoadTagEnabled = allLoadTagEnabled;
    }
...
//    }
    ...
    protected class InternalListView extends ListView implements EmptyViewMethodAccessor {
        ...
        public void setAdapter(ListAdapter adapter) {
            ...
            if (mAllLoadTagEnabled && frameLayout_bottomResultHint == null) {
                TextView textView_FootResultHint = new TextView(getContext());
                textView_FootResultHint.setTextColor(Color.parseColor("#C1CADB"));
                textView_FootResultHint.setGravity(Gravity.CENTER);
                textView_FootResultHint.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
                textView_FootResultHint.setText(R.string.no_more_data);
                textView_FootResultHint.setVisibility(View.GONE);
                frameLayout_bottomResultHint = new FrameLayout(getContext());
                int padding = (int) (getContext().getResources().getDisplayMetrics().density * 5);
                frameLayout_bottomResultHint.setPadding(0, padding, 0, padding);
                frameLayout_bottomResultHint.addView(textView_FootResultHint);
            }

            if (mAllLoadTagEnabled && frameLayout_bottomResultHint.getParent() == null)
                addFooterView(frameLayout_bottomResultHint);
    ...
    @Override
    public void setFootReboundInsteadLoad(boolean foodReboundInsteadLoad) {
        if (frameLayout_bottomResultHint != null) {
            ListView listView = getRefreshableView();
            boolean showResultHint = (foodReboundInsteadLoad && listView.getAdapter() != null && (listView.getAdapter().getCount() - listView.getHeaderViewsCount() - listView.getFooterViewsCount()) > 0);
            frameLayout_bottomResultHint.getChildAt(0).setVisibility(showResultHint ? View.VISIBLE : View.GONE);
        }
        super.setFootReboundInsteadLoad(foodReboundInsteadLoad);
    }
}